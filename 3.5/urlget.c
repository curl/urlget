 ...
 *
 *  TODO:
 *   - (HTTP) Add support for PUT, POST, TRACE, DELETE and conditional GETs.
 *   - (FTP) Make a get of a directory use the LIST/NLST command to get the
 *     directory list.
 *
 */
          ...
          if(!str) {
            ...
            do {
              ...
              if(!strlen(hbufp)) {
                ...
                str+=2; /* pass the null byte and the followin \n byte */
    ...
    if(strncmp(buf, "227", 3)) {
      ...
      if(conf & CONF_UPLOAD) {
        ...
        for (;;) {
          ...
          nread = data->fread(buf, 1, BUFSIZE, data->in);
          bytecount += nread;
          ProgressShow(data, bytecount, now, time(NULL));
          ...
          /* write to socket */
          if(nread != swrite(data->secondarysocket, buf, nread)) {
            failf(data, "Failed uploading file");
            /* hm, open sockets never closed! */
            return URG_FTP_WRITE_ERROR;
          }
